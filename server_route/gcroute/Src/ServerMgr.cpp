...
INT CServerMgr::Open(int port)
{
    ...
    CServerToKafka::GetInstance().InitGcApnsTopic();
    ...
    {
        CConsumeDealMsgThread * pCConsumeDealMsgThread = new CConsumeDealMsgThread(this, i, moConfig.m_sapptype);
...
void CServerMgr::AddTailMsg(StrConsumeMsg* msg)
{
    ...
    std::size_t groupendpos = msg->szkey.find('_');
    szgroupid =  msg->szkey.substr(groupstartpos, groupendpos - groupstartpos); 
    UINT64 groupid = ServerUtilMgr::STR2UINT64(szgroupid);

    miCurInex = groupid % moConfig.m_consumethreadcnt;
...
UINT64 CServerMgr::GetDealConsumeOnlineCnt()
{
    UINT64 msgcnt = 0;

    for (auto it = mpCConsumeDealMsgThreadlist.begin(); it != mpCConsumeDealMsgThreadlist.end(); it++)
    {
        msgcnt += (*it)->GetOnlineCnt();
    }
