...
bool CServerMgr::Send2Comet(uint32 threadid, const SERVERID & serverid , BYTE * pdata,uint32 uidatalen)
{
    
    if (NULL != mpClientSide)
    {
        return mpClientSide->Send2Comet(threadid, serverid, pdata, uidatalen);
    }
...
void CServerMgr::Getuseridlogins(std::vector<USERID> & userids, 
...
{
    return mpClientSide->Getuseridlogins(userids,
                                           szappserverid_userid_map, 
                                           szappserverids,
                                           pofflineuserlist,
                                           offlineusercnt,
                                           apptype);
