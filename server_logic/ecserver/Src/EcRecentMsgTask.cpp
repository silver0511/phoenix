...
void CEcRecentMsgTask::DoForwardPackageFail()
{    
    LOG_TRACE(LOG_ERR, false, __FUNCTION__, "Forward package fail url = " << EC_RECENTMSG_API);
    ...
    ecpack::T_EC_GETRECENTMSGS_RSBuilder c1 = ecpack::T_EC_GETRECENTMSGS_RSBuilder(loSendPack.fbbuilder);
    c1.add_s_rs_head(&s_rs_head);
    c1.add_b_id(m_bid);
    c1.add_c_id(m_cid);
    c1.add_message_id(m_msgid);
...
void CEcRecentMsgTask::run()
{
    ...
    std::string szUrl = EC_RECENTMSG_API + "?" + m_data;
