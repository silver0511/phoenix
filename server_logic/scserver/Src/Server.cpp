...
{//退出信号
	LOG_TRACE(LOG_ERR,true, __FUNCTION__,"SESSION OUT:" << code);
}
...
void Task(int port, int log_type)
{
	...
	{
		delete lpServer;
		LOG_TRACE(LOG_ERR, true, __FUNCTION__, g_server_name + " Start failt Server .....");
		return ;
	}
	LOG_TRACE(LOG_DEBUG, true, __FUNCTION__, g_server_name + " Server Open Success");
	...
	LOG_TRACE(LOG_DEBUG,true, __FUNCTION__,"Exit Server .");
	lpServer->Close();
	delete lpServer;
	LOG_CLOSE();
}
