...
case $1 in
        ...
        then
        	...
        	nohup java $CMD_ARGS -javaagent:/data/cat/bin/apm-core-1.0.0.jar -DAPM_HOME=/data/cat -DUCAT_HOME=/data/cat -Dcom.qbao.apm.weaver.showWeaveInfo=true -jar im_server-1.0-SNAPSHOT.jar &
        	sleep 2
        	ps=`ps -fe |grep jar|grep im_server|grep -v grep|tr -s ' '|awk -F' ' '{print $2}'`
        	if [ -n "$ps" ]
        	then
