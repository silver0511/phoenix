...
#define BIND_MULITARRAY(val) std::bind(&FetchMulitArray, std::placeholders::_1, val)
...
static inline int FetchMulitArray(redisReply *pReply, std::vector<std::string> *pvecStrVal)
{
    if (pReply->type == REDIS_REPLY_ARRAY)
    {
        int nRet = RC_SUCCESS;
        ...
        {
            return RC_PART_SUCCESS;
        }

        int nSubRet = FetchString(pReply->element[0], &strVal);
        ...
        {
            nSubRet = FetchString(pReply->element[1]->element[i], &strVal);
            if (nSubRet == RC_SUCCESS)
                pvecStrVal->push_back(strVal);
            else
                nRet = RC_PART_SUCCESS;
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::string &strArg2, const std::string &strArg3, const std::string &strArg4, const std::string &strArg5, const std::string &strArg6)
{
    InitMemory(7);
    AppendValue(strArg1);
    AppendValue(strArg2);
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::string &strArg2, const std::string &strArg3, const std::string &strArg4, const std::string &strArg5, const std::string &strArg6, const std::string &strArg7)
{
    InitMemory(8);
    AppendValue(strArg1);
    AppendValue(strArg2);
...
int CRedisCommand::CmdRequest(redisContext *pContext)
{
	...
	char* strE= "evalsha";
    char* strP = "script load";
	if (strcmp(*(m_pszArgs+0),strE) == 0 )
	{
		std::string dostr = *(m_pszArgs+1);
		std::string agvstr = *(m_pszArgs+2);
		std::string tmpstr = "evalsha ";  
		tmpstr += agvstr;  
		tmpstr += " ";
		tmpstr += dostr;
		redisLuaAppendCommand(pContext, tmpstr.c_str());  
		if (redisGetReply(pContext, (void**)&m_pReply) != REDIS_OK)  
			return RC_RQST_ERR; 
	}
	else if (strcmp(*(m_pszArgs+0),strP) == 0 )
	{
		std::string filestr = *(m_pszArgs+1);
		redisLoadLuaAppendCommand(pContext,"script load \"%s\"",filestr.c_str());
		if (redisGetReply(pContext, (void**)&m_pReply) != REDIS_OK)  
			return RC_RQST_ERR; 
	}
	else
...
{
    return ExecuteImpl("lpush", strKey, vecVal, HASH_SLOT(strKey), ppLine, BIND_INT(nullptr));
    // return ExecuteImpl("lpush", strKey, vecVal, HASH_SLOT(strKey), ppLine);
}
...
{
    return ExecuteImpl("rpush", strKey, vecVal, HASH_SLOT(strKey), ppLine, BIND_INT(nullptr));
}
...
int CRedisClient::Sadd(const std::string &strKey, const std::vector<std::string> &vector, long *pnVal, Pipeline ppLine)
{
    return ExecuteImpl("sadd", strKey, vector, HASH_SLOT(strKey), ppLine, BIND_INT(pnVal));
}
...
int CRedisClient::Hsan(const std::string &strkey, std::string& pCursor,  long nCount, std::map<std::string, std::string> *pmapVal, Pipeline ppLine)
{
    std::vector<std::string> vecVal;
    int nRet = ExecuteImpl("hscan", strkey, ConvertToString(pCursor), "count", ConvertToString(nCount), HASH_SLOT(strkey), ppLine,BIND_MULITARRAY(&vecVal));
    ...
    if (nRet == RC_SUCCESS)
    {
        if (pmapVal)
        {
            ...
            for (size_t i = 1; i < vecVal.size() - 1; i+=2)
            {
                pmapVal->insert(std::make_pair(vecVal[i], vecVal[i+1]));
            }
        }
    }
...
int CRedisClient::Hset(const std::string &strKey, const std::string &strField, const std::string &strVal, long *pnVal, Pipeline ppLine)
{
    return ExecuteImpl("hset", strKey, strField, strVal, HASH_SLOT(strKey), ppLine, BIND_INT(pnVal));
}
...
int CRedisClient::Zadd(const std::string &strKey, unsigned long long dScore, const std::string &strElem, long *pnVal, Pipeline ppLine)
{
    return ExecuteImpl("zadd", strKey, ConvertToString(dScore), strElem, HASH_SLOT(strKey), ppLine, BIND_INT(pnVal));
}
...
int CRedisClient::Zrangebyscore(const std::string &strKey, unsigned long long dMin, int nSpan, std::vector<std::string> *pvecVal, Pipeline ppLine)
{
    return ExecuteImpl("zrangebyscore", strKey, ConvertToString(dMin), ConvertToString("+inf"), ConvertToString("limit"), ConvertToString("0"), ConvertToString(nSpan),  HASH_SLOT(strKey), ppLine, BIND_VSTR(pvecVal));
}

int CRedisClient::ZrangeWithScorebySpan(const std::string &strKey, unsigned long long dMin, int nSpan, std::map<std::string, std::string> *pmapVal, Pipeline ppLine)
{
    std::map<std::string, std::string> mapVal;
    int nRet = ExecuteImpl("zrangebyscore", strKey, ConvertToString(dMin), ConvertToString("+inf"), ConvertToString("limit"), ConvertToString("0"), ConvertToString(nSpan),  std::string("WITHSCORES"), HASH_SLOT(strKey), ppLine, BIND_MAP(&mapVal));

    if (nRet == RC_SUCCESS && pmapVal)
    {
        for (auto &memPair : mapVal)
            pmapVal->insert(std::make_pair(memPair.second, memPair.first));
    }
...
int CRedisClient::Zrangebyscore(const std::string &strKey, unsigned long long dMin, unsigned long long dMax, std::map<unsigned long long, std::string> *pmapVal, Pipeline ppLine)
{
    std::map<std::string, std::string> mapVal;
    int nRet = ExecuteImpl("zrangebyscore", strKey, ConvertToString(dMin), ConvertToString(dMax), std::string("WITHSCORES"), HASH_SLOT(strKey), ppLine, BIND_MAP(&mapVal));
    if (nRet == RC_SUCCESS && pmapVal)
    {
        ...
        for (auto &memPair : mapVal)
        {
            unsigned long long message_id = strtoull(memPair.second.c_str(), nullptr, 10);
            pmapVal->insert(std::make_pair(message_id, memPair.first));
        }
    }
    return nRet;
}
...
int CRedisClient::Zremrangebyscore(const std::string &strKey, unsigned long long dMin, unsigned long long dMax, long *pnVal, Pipeline ppLine)
{
    return ExecuteImpl("zremrangebyscore", strKey, ConvertToString(dMin), ConvertToString(dMax), HASH_SLOT(strKey), ppLine, BIND_INT(pnVal));
}
...
int CRedisClient::Zrevrangebyscore(const std::string &strKey, unsigned long long dMax, int nSpan, std::vector<std::string> *pvecVal, Pipeline ppLine)
{
    return ExecuteImpl("zrevrangebyscore", strKey, ConvertToString(dMax), ConvertToString("-inf"), ConvertToString("limit"), ConvertToString("0"), ConvertToString(nSpan), HASH_SLOT(strKey), ppLine, BIND_VSTR(pvecVal));
}

int CRedisClient::Zrevrangebyscore(const std::string &strKey, unsigned long long dMax, int nSpan, std::map<unsigned long long, std::string> *pmapVal, Pipeline ppLine)
{
    std::map<std::string, std::string> mapVal;
    int nRet = ExecuteImpl("zrevrangebyscore", strKey, ConvertToString(dMax), ConvertToString("-inf"), ConvertToString("limit"), ConvertToString("0"), ConvertToString(nSpan), std::string("WITHSCORES"), HASH_SLOT(strKey), ppLine, BIND_MAP(&mapVal));
    if (nRet == RC_SUCCESS && pmapVal)
    {
        pmapVal->clear();
        for (auto &memPair : mapVal)
...
int CRedisClient::Scriptload(const std::string &strKey, std::string *pstrVal, Pipeline ppLine)
{
	return ExecuteImpl("script load", strKey," ", HASH_SLOT(strKey), ppLine, BIND_STR(pstrVal));
}

int CRedisClient::Evalsha(const std::string &strKey, const std::string &strField, std::string *pstrVal, Pipeline ppLine)
{
	return ExecuteImpl("evalsha", strKey, strField, HASH_SLOT(strKey), ppLine, BIND_STR(pstrVal));
}

int CRedisClient::Evalsha(const std::string &strKey, const std::string &strField, std::vector<std::string> *pvecVal, Pipeline ppLine)
{
	return ExecuteImpl("evalsha", strKey, strField, HASH_SLOT(strKey), ppLine, BIND_VSTR(pvecVal));
}