...
{
	//reg
	ServerPack::GetInstance().AddRouter(NEW_DEF_SMS_VALID_RQ, REG_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_REG_RQ, REG_SERVER);
	...
	ServerPack::GetInstance().AddRouter(NEW_DEF_REGISTER_APNS_RQ, USER_SERVER);
	...
	ServerPack::GetInstance().AddRouter(NEW_DEF_SINGLE_CHAT_STATUS_RQ, SC_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GET_USER_STATUS_RQ, SC_SERVER);
	...
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_UPDATE_LOGOUT_ID, GC_SERVER);
	...
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_LIST_IDS_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_MESSAGE_STATUS_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_TYPE_LIST_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_GET_USER_REMARK_NAME_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_SAVE_CHANGE_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_SCAN_RQ, GROUP_SERVER);
	ServerPack::GetInstance().AddRouter(NEW_DEF_GROUP_BATCH_INFO_RQ, GROUP_SERVER);
	...
	//ec
    ServerPack::GetInstance().AddRouter(NEW_DEF_BUSINESS_GETFREEWAITER_RQ, EC_SERVER);
    ServerPack::GetInstance().AddRouter(NEW_DEF_BUSINESS_SETINFO_RQ, EC_SERVER);
    ServerPack::GetInstance().AddRouter(NEW_DEF_BUSINESS_GETINFO_RQ, EC_SERVER);
	...
	ServerPack::GetInstance().AddRouter(NEW_DEF_SERVER_OFFCIAL_PRIVATE_MESSAGE_RS, OFFCIAL_SERVER);
...
BOOL CServerSide::OpenLogicServerPool()
{
	...
	for(map_iter = mMapServerLogicConfig.begin(); map_iter != mMapServerLogicConfig.end(); ++map_iter)
	{
		...
		for(server_iter = list_server_config_info.begin(); server_iter != list_server_config_info.end(); ++server_iter)
		{
			...
			{
				LOG_TRACE(LOG_CRIT,true,__FUNCTION__," Initialize logic user tcp pool error.");
				return false;
			}
		}
	}
...
INT CServerSide::OnRecvData(SOCKET ahSocket, IPTYPE aiIP,WORD awPort,BYTE* apData,WORD awLen)
{
	...
	for (int user_index = 0; user_index < loHeadSvrPacket.miUserCount; ++user_index)
	{
		...
		if(loHeadSvrPacket.mwPackType == NEW_DEF_SMS_VALID_RS ||
				loHeadSvrPacket.mwPackType == NEW_DEF_REG_RS)
		{
			lpUser = mpServerMgr->moRegUserMgr.delUser(user_id);
		}
		...
		if(loHeadSvrPacket.mwPackType == STRU_SERVER_DISCON_ID::ENUM_TYPE)
		{
			lpUser->miLoginCount++;
