...
private:
	...
	INT UpdateTimeID(USERID user_id);
...
public:
	...
	virtual INT OnNewRecvData(SOCKET ahSocket, void* apRelationPtr,IPTYPE aiIP,WORD awPort, BYTE* apData,WORD awLen)
	{
        ...
        else if(NEW_DEF_TIME_SYNC_RQ == loHeadPackage.mwPackType)
        {
            return this->onRecvTimeSyncRQ(loHeadPackage.mwPackType,ahSocket,apRelationPtr,aiIP,awPort,apData,awLen);
        }
		else if(NEW_DEF_REG_RQ == loHeadPackage.mwPackType)
		{
			return this->onRecvTransmitRQ2(loHeadPackage.mwPackType,ahSocket,apRelationPtr,aiIP,awPort,apData,awLen);
		}
		else if(NEW_DEF_SMS_VALID_RQ == loHeadPackage.mwPackType)
		{
			return this->onRecvTransmitRQ2(loHeadPackage.mwPackType,ahSocket,apRelationPtr,aiIP,awPort,apData,awLen);
		}
		else
...
private:
    ...
    INT onRecvTimeSyncRQ(WORD awPackType, SOCKET ahSocket,void* apRelationPtr,IPTYPE aiIP, WORD awPort, BYTE * apData, WORD awLen);
	INT onRecvTransmitRQ(WORD awPackType, SOCKET ahSocket,void* apRelationPtr,IPTYPE aiIP, WORD awPort, BYTE * apData, WORD awLen);
	INT onRecvTransmitRQ2(WORD awPackType, SOCKET ahSocket,void* apRelationPtr,IPTYPE aiIP, WORD awPort, BYTE * apData, WORD awLen);
public:
private:
